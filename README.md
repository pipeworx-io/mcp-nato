...
`tools/list` at `https://gateway.pipeworx.io/nato/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/spell_nato \
  -H 'Content-Type: application/json' \
  -d '{"text":"ALPHA123"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/spell_nato`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
